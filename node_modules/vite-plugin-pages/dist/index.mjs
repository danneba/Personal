...
function generateRoutes(pages, options) {
  ...
  const allRoute = finalRoutes.find((i) => isCatchAllRoute(basename2(i.component)));
  if (allRoute) {
    finalRoutes = finalRoutes.filter((i) => !isCatchAllRoute(basename2(i.component)));
...
function resolvePages(options) {
  ...
  const pageDirFiles = dirs.map((pageDir) => {
    const pagePath = slash(resolve3(options.root, pageDir.dir));
...
function setPage(pages, pageDir, file, options) {
  ...
  const filepath = slash(resolve3(options.root, component));
